...
AdvancedFlagKind = Literal[
    "bool", "text", "enum", "int", "duration_minutes",
    "extensions", "csv_repeat", "lines_repeat", "date_range",
    "path", "paths",
]
...
class Registry:
    ...
    def simple_keys(self, tab_key: str) -> set[str]:
        ...
        return {
            d.key for d in self.flags.get(tab_key, ())
            if d.mode == "simple"
        }

    def advanced_keys(self, tab_key: str) -> set[str]:
        """Keys that are advanced-only (mode advanced)."""
        return {
            d.key for d in self.flags.get(tab_key, ())
            if d.mode == "advanced"
        }
